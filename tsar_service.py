...
#favsel{min-width:170px}
.star{background:none;border:0;cursor:pointer;font-size:18px;line-height:1;padding:0 2px;margin-right:-4px;color:var(--dim)}
...
.stepnote{flex-basis:100%;font-size:12px;color:var(--dim);margin:2px 0 4px 32px;line-height:1.45}
.step.skipped .stepno{border-style:dashed}
...
<header class="mast">
  ...
  <div class="controls">
    ...
    <div class="field" id="favfield">
      <label>Favorites</label>
      <select class="inp" id="favsel"></select>
    </div>
...
<div class="intro" id="intro">
  <div class="introcard">
    ...
    <div class="setup" id="setup">
      ...
      <div class="step" id="sstep2" hidden><span class="stepno"><span>2</span></span><button class="gobtn" id="setupini">Choose FYMMyMaps.ini</button><button class="sib" id="setupskip">Skip — I'll star my yards myself</button>
        <div class="stepnote" id="sstep2t">Windows lets the browser read the folder but not its .ini files, so this one file is picked on its own
          and then remembered. <b>FYMMyMaps.ini</b> is the game's list of the yards assigned to you: with it, your yards are found
          and starred for you. Without it, the yards you star ☆ become your list. Every yard's cars are readable either way.</div></div>
...
function toggleFav(id){
  ...
  if(GAME.kind&&GAME.mySource==="favs"){ GAME.my=myFromFavs(); wagFor(id); paintGameBtn(); }
}
const myFromFavs=()=>new Set(favs.filter(id=>GAME.visited.has(id)));
...
// The Favorites dropdown is always present so the feature is discoverable;
// with nothing starred its only option says how to add one. Picking a yard
// focuses it exactly like a route-node click, then the select springs back to
// its placeholder so the same yard can be picked again after clearing.
const favsel=document.getElementById("favsel");
function paintFavs(){
  const ids=favs.filter(id=>KNOWN_LOC.has(id));
  favsel.innerHTML = ids.length
    ? `<option value="">${ids.length} saved yard${ids.length>1?"s":""}…</option>`+
      ids.map(id=>`<option value="${id}">${esc(locLabel(id))} (${locCount[id]||0})</option>`).join("")
    : `<option value="">none yet — ☆ a yard to save it</option>`;
  favsel.disabled = !ids.length;
}
favsel.onchange=()=>{ const id=favsel.value; favsel.value=""; if(id) gotoLoc(id); };
...
function render(reset=true){
  ...
  if(state.loc){
    ...
    locbar.innerHTML=`<button class="star${isFav(L)?' on':''}" id="favstar" title="${isFav(L)?'remove from':'add to'} favorites">${isFav(L)?'★':'☆'}</button>`+
      `<span class="lid">#${L}</span>`+
...
async function loadGame(){
  ...
  try{ mm=await gameText("FYMMyMaps.ini"); }
    ...
    if(isNameBlocked(e)){        // Windows Chrome/Edge: folder is fine, the one personal .ini needs its own picker
      if(!GAME.skipIni){ GAME.kind=null; GAME.needIni=true; paintGameBtn(); render(); gameNote(""); showIntro(true); return; }
      mm=null;                   // skipped: the starred yards stand in for the assigned list
    } else {
  ...
  if(GAME.mySource==="favs") GAME.my=myFromFavs();
  else {
    let added=0;
...
function paintSetup(){
  const n=GAME.my.size, skip=document.getElementById("setupskip"), s2t=document.getElementById("sstep2t");
  if(GAME.kind){
    sstep1.classList.add("done"); setupdir.disabled=true; setupdir.textContent="Folder open";
    if(GAME.mySource==="favs"){
      sstep1t.textContent=(GAME.name?GAME.name+" · ":"")+n+" starred yard"+(n===1?"":"s")+" with an inventory file";
      sstep2.hidden=false; sstep2.classList.add("skipped"); skip.hidden=true;
      s2t.innerHTML="Skipped. Your list is the yards you star ☆ from the location list. Choose <b>FYMMyMaps.ini</b> at any time to have the game's assigned yards found and starred for you instead.";
    } else {
    ...
    sstep1.classList.add("done"); setupdir.disabled=true; setupdir.textContent="Folder open";
    sstep1t.textContent=GAME.name||""; sstep2.hidden=false; sstep2.classList.remove("skipped"); skip.hidden=false;
    s2t.innerHTML="Windows lets the browser read the folder but not its .ini files, so this one file is picked on its own and then remembered. <b>FYMMyMaps.ini</b> is the game's list of the yards assigned to you: with it, your yards are found and starred for you. Without it, the yards you star ☆ become your list. Every yard's cars are readable either way.";
  } else {
...
async function skipIni(){
  GAME.skipIni=true; await idbSet("skipini",true);
  GAME.needIni=false; GAME.kind="handle";
  gameNote("Reading your folder…"); await loadGame();
}
document.getElementById("setupskip").onclick=()=>skipIni();
gamebtn.onclick=async()=>{
  ...
  try{
    ...
    if(!h){ h=await window.showDirectoryPicker({mode:"read"}); await idbSet("game",h); await idbDel("ini"); await idbDel("initext"); await idbDel("skipini"); GAME.ini=null; GAME.skipIni=false; }
...
function myYardsPanel(){
  ...
  const fromFavs=GAME.mySource==="favs";
  let h=`<h4>My yards <span class="dim">· ${ids.length} ${fromFavs?"starred with an inventory file here":"assigned in FYMMyMaps.ini"}</span></h4>`;
  if(!ids.length) h+=fromFavs?`<div class="drow dim">Star ☆ a yard you have opened in the game and it appears here.</div>`
                            :`<div class="drow dim">No yards are flagged as yours in FYMMyMaps.ini.</div>`;
  if(GAME.ini){
...
function navState(){
  return {i:navIdx, loc:state.loc, locmode:state.locmode, view:state.view, sym:state.sym, stat:state.stat,
...
function navHash(s){
  ...
  if(s.sym) p.push("sym="+encodeURIComponent(s.sym));
...
function navApply(s){
  ...
  try{
    ...
    state.sym=s.sym||""; symin.value=state.sym; setStat(s.stat||"active");

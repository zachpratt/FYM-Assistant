...
async function disconnectGame(){
  await idbDel("game");
  Object.assign(GAME,{kind:null,root:null,files:null,stored:null,my:new Set(),visited:new Set(),types:{},parent:{},wags:{},saved:{}});
...
function carRow(c,L){
  ...
  const via=!hits?"":hits.length?trainChip(hits[0])+(hits.length>1?`<span class="dim"> +${hits.length-1}</span>`:"")
...
function yardPanel(L){
  ...
  h+=groups.map(g=>`<h5 class="ygh">${g.dest?`<button class="sib" data-loc="${g.dest}">${esc(g.title)}</button>`:esc(g.title)}`+
    `<span class="dim">${g.cars.length} car${g.cars.length===1?"":"s"}${g.sub?" · "+esc(g.sub):""}</span>${groupTrains(g)}</h5>`+

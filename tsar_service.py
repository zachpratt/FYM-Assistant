...
async function loadGame(){
  ...
  try{   // FYMStates.ini: StateN=XX:colour:region:Name — sort tokens 1..58 are these N
    ...
    (await gameText("FYMStates.ini")).split(/\r?\n/).forEach(l=>{
...
// trains from this yard that carry a sort's block, scored by how much of the
// sort's definition they cover; name words are only a last resort
function sortTrains(sl,L,atL){
  const Lm=mapOf(L);
  // an industry-only sort is a local spot, not a block for a train
  if(sl.inds.size&&!sl.ids.size&&!sl.states.size&&!sl.rr.length&&!sl.flags.has("62")) return {hits:[],byName:false,local:true};
  ...
  if(uniq.length) return {hits:uniq.slice(0,4),byName:false};
  // fallback: a catch-all or state-only sort whose name carries a place
  ...
  return {hits:byName.slice(0,4),byName:true};
}
...
function yardPanel(L){
  ...
  const groupTrains=g=>{
    if(g.slot){
      ...
      return `<span class="ytrains">${st.byName?'<span class="dim" title="matched on the sort name only">by name: </span>':""}`+
